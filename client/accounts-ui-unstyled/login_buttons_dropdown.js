(function () {
  ...
  Template._loginButtonsLoggedOutPasswordService.fields = function () {
    var loginFields = [
      ...
      {fieldName: 'password', fieldLabel: 'Mot de passe', inputType: 'password',
       visible: function () {
         return true;
       }}
    ];
    ...
    if (email.indexOf('@') !== -1) {
      Accounts.forgotPassword({email: email}, function (error) {
        if (error)
          ...
          Accounts._loginButtonsSession.set('infoMessage', "Email sent");
      });
    } else {
      Accounts._loginButtonsSession.set('errorMessage', "Invalid email");
    }
  };
  ...
  var changePassword = function () {
    ...
    Accounts.changePassword(oldPassword, password, function (error) {
      if (error) {
        ...
        Accounts._loginButtonsSession.set('infoMessage', "Password changed");
      }
    });
  };
  ...
  var matchPasswordAgainIfPresent = function () {
    ...
    if (passwordAgain !== null) {
      ...
      if (password !== passwordAgain) {
        Accounts._loginButtonsSession.set('errorMessage', "Passwords don't match");
        return false;
      }
    }
    return true;
